...
#import "DoPhotoBrowserPrivate.h"
...
@implementation DoPhotoBrowser
{
    DoSelectBtn* _selectBtn;
    
    DoSureBtn* _doSureBtn;
}
#pragma mark - Init
...
- (id)initWithDelegate:(id <DoPhotoBrowserDelegate>)delegate {
    if ((self = [self init])) {
        _delegate = delegate;
	}
	return self;
...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        _selectBtn = [DoSelectBtn instance];

        [_selectBtn.selectBtn addTarget:self action:@selector(selectBtnClick:) forControlEvents:UIControlEventTouchUpInside];
//        _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(actionButtonPressed:)];
        _actionButton = [[UIBarButtonItem alloc] initWithCustomView:_selectBtn];
...
//    titleLabel.font = XIAOYUFONT(16);
    titleLabel.font = [UIFont systemFontOfSize:16];
    self.navigationItem.titleView = titleLabel;
    
    [_doSureBtn setNum:_selectImgArray.count];
	// Super
    [super viewDidLoad];
...
- (void)performLayout {
    ...
    if (_actionButton && !(!hasItems && !self.navigationItem.rightBarButtonItem)) {
        _doSureBtn = [DoSureBtn instance];
        _doSureBtn.maxCount = _maxNum;
        [_doSureBtn.sureBtn addTarget:self action:@selector(sureBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [items addObject:[[UIBarButtonItem alloc] initWithCustomView:_doSureBtn]];
    } else {
...
- (id<MWPhoto>)photoAtIndex:(NSUInteger)index {
    ...
    for (int i=0; i<_selectImgArray.count; i++) {
        ...
        //设置右上角的数字
        if ([[_selectImgArray objectAtIndex:i] integerValue] == self.currentIndex) {
            isSelect = YES;
            for (int j=0; j<_selectImgArray.count; j++) {
                if ([[_selectImgArray objectAtIndex:j] integerValue] == self.currentIndex) {
                    [_selectBtn setNum:[_selectImgArray indexOfObject:[_selectImgArray objectAtIndex:j]]+1];
                }
            }
...
-(void)selectBtnClick:(id)sender
{
    ...
    if (!_selectBtn.selectBtn.selected) {
        if (_selectImgArray.count >= _maxNum) {
//            [RTUtil showStatusBarWarning:[NSString stringWithFormat:@"最多只能选择%ld张",_maxNum]];
            return;
        }
        [_selectBtn setNum:_selectImgArray.count+1];
        [_selectImgArray addObject:[NSNumber numberWithInt:self.currentIndex]];
    }else{
        for (int i=0; i<_selectImgArray.count; i++) {
            NSNumber* num = [_selectImgArray objectAtIndex:i];
            if (num.integerValue == self.currentIndex) {
                [_selectImgArray removeObjectAtIndex:i];
                [_selectBtn setSelectMode:NO];
            }
        }
    }
...
{
    if ([_delegate respondsToSelector:@selector(selectImageIndexArray:)]) {
        [_delegate selectImageIndexArray:_selectImgArray];
    }
